...
form.addEventListener("submit", async (e) => {
  ...
  var publicLink =
    typeof domkaPublicUrl === "function"
      ? domkaPublicUrl("public/cotizacion.html?id=" + docRef.id)
      : (window.DOMKA_PUBLIC_BASE || "") + "/public/cotizacion.html?id=" + docRef.id;

  await db.collection("cotizaciones").doc(docRef.id).update({
    linkPublico: publicLink
  });
