...
(() => {
  ...
  let _current = null;
  let _perfil  = null;
  let _usuarios = []; // lista global de usuarios del sistema
  ...
  function render(list) {
    ...
    tbody.innerHTML = list.map((p, idx) => {
      ...
      const asocCount = (p.usuariosAsociados || []).length;
      return `
        <tr class="border-b hover:bg-gray-50 cursor-pointer" onclick="verDetalle('${p.id}')">
          ...
          <td class="p-3 text-xs text-gray-500">
            ...
            ${docCount ? `<span class="mr-2">📄 ${docCount}</span>` : ""}
            ${asocCount ? `<span>👥 ${asocCount}</span>` : ""}
            ${(!evCount && !docCount && !asocCount) ? "—" : ""}
          </td>
  ...
  window.sincronizarAprobadores = function() {
    const wrapA = document.getElementById("proy-aprobadores-wrap");
    if (!wrapA) return;
    const seleccionados = Array.from(document.querySelectorAll(".proy-user-chk:checked"));
    if (!seleccionados.length) {
      wrapA.innerHTML = `<p class="text-gray-400 text-xs italic px-1">Primero selecciona usuarios asociados</p>`;
      return;
    }
    wrapA.innerHTML = seleccionados.map(chk => `
      <label class="flex items-center gap-2 px-1 py-1 hover:bg-green-50 rounded cursor-pointer">
  ...
  async function crearProyecto(e) {
    ...
    formProyecto.reset();
    renderUsuariosPicker(); // reset checkboxes
    await cargar();
  }
  ...
  function canApprove(p) {
    ...
    const aprobadores = p.aprobadores || [];
    return aprobadores.some(a => a.uid === uid);
  }
  ...
  window.aprobarDoc = async function(proyId, docNombre) {
    ...
    const docs = (data.documentos || []).map(d =>
      (d.nombre === docNombre || d.tipo === docNombre) ? { ...d, estado: "aprobado", aprobadoPor: window.auth?.currentUser?.uid, aprobadoEn: new Date().toISOString() } : d
    );
    await db.collection("proyectos").doc(proyId).update({ documentos: docs });
    const updated = { id: proyId, ...snap.data(), documentos: docs };
    _all = _all.map(p => p.id === proyId ? updated : p);
    _current = updated;
  ...
  /* ── Documentos del proyecto (admin) — ahora con archivo ── */
